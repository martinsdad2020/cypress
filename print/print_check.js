...
describe("Check", function() {
   ...
   it('Check', function() {
   	cy.get('.nav').contains('Счета').click();
      cy.get('.tab-pane.active > .panel.ng-scope').contains('0,00').should('be.visible');
      cy.get('.tab-pane.active > .panel.ng-scope > [filter-fields="$parent.undefined"] > .panel-footer').contains('Печать').click();
      cy.get('[style="padding-top: 40px;"] > .collectionTable > .collectionTable__wrapper').contains('0,00').should('be.visible');
