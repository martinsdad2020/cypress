...
describe("Delivery", function() {
   ...
   it('Delivery', function() {
   	cy.get('.nav').contains('Доставки').click();
      cy.get('.tab-pane.active > .panel.ng-scope').contains('1,00').should('be.visible');
      cy.get('.tab-pane.active > .panel.ng-scope > [filter-fields="$parent.undefined"] > .panel-footer').contains('Печать').click();
      cy.get('[style="padding-top: 40px;"] > .collectionTable > .collectionTable__wrapper').contains('1,00').should('be.visible');
