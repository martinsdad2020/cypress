...
describe("repeat", function() {
   ...
   it('repeat', function() {
   	cy.get('.nav').contains('обращения').click();
      cy.get('.tab-pane.active > .panel.ng-scope').contains('Второй').should('be.visible');
      cy.get('.tab-pane.active > .panel.ng-scope > [filter-fields="$parent.undefined"] > .panel-footer').contains('Печать').click();
      cy.get('[style="padding-top: 40px;"] > .collectionTable > .collectionTable__wrapper').contains('Второй').should('be.visible');
