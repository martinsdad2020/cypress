...
describe("history search", function() {
   ...
   it('history search', function() {
   	cy.get('.nav').contains('История поиска').click();
      cy.get('.tab-pane.active > .panel.ng-scope').contains('Jaguar').should('be.visible');
      cy.get('.tab-pane.active > .panel.ng-scope > [table-template="template"] > .panel-footer').contains('Печать').click();
      cy.get('[style="padding-top: 40px;"] > .collectionTable > .collectionTable__wrapper').contains('Jaguar').should('be.visible');
