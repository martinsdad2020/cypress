...
describe("Payments", function() {
   ...
   it('Payments', function() {
   	cy.get('.nav').contains('Платежи').click();
      cy.get('.tab-pane.active > .panel.ng-scope').contains('Банковская карта').should('be.visible');
      cy.get('.tab-pane.active > .panel.ng-scope > [filter-fields="$parent.undefined"] > .panel-footer').contains('Печать').click();
      cy.get('[style="padding-top: 40px;"] > .collectionTable > .collectionTable__wrapper').contains('Банковская карта').should('be.visible');
